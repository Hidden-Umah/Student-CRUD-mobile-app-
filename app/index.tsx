...
export default function Home() {
  ...
  const sidebarItems = [
    { id: 1, icon: <AntDesign name="user-add" size={26} color="#09090f" />, backgroundColor: "#d2f17bff" },
    { id: 2, icon: <Ionicons name="people-outline" size={26} color="#09090f" />, backgroundColor: "#7bb8f1ff" },
    { id: 3, icon: <Feather name="edit-2" size={26} color="white" />, backgroundColor: "#f8b02bff" },
    { id: 4, icon: <MaterialCommunityIcons name="delete-outline" size={30} color="white" />, backgroundColor: "#eb4343ff" },
  ]
...

            {
              ...
              <View style={{ height: 70 , width: width*0.9 , marginTop: 20 , display: 'flex' , flexDirection: 'row' , alignItems: 'center' , justifyContent: 'space-evenly'}}>
                {sidebarItems.map((item ,index) => (
                  <View key={index} style={{ width: ICON_SIZE, height: ICON_SIZE, backgroundColor: item.backgroundColor, borderRadius: 22, alignItems: "center", justifyContent: "center" }}>
                   {item.icon}
                  </View>
                ))}
              </View>
